...
#define g(x) ((-1)*sqrt(x+6))
#define gd(x0) (-0.5/sqrt(x0+6))
#define f(x) (x*x-x-6)
...
int main()
{
	...
	do{
		...
		{
			if(gd(x0)>1){ //checking convergence of equation
	...
	}while(error>0.000001 && count<MAX_COUNT);	//Terminating condition
